...
func Version(packageName,version, gitDescribe, gitHash string) *VersionInfo {
...
func (v *VersionInfo) Print(maker string) {
	log.Infof(v.PackageName, "version:", v.Version)
	log.Infof("Made By:", maker)
	log.Infof("")

	log.Infof("Build information:")
	fmt.Printf("  Go version: %s (%s, %s)\n", runtime.Version(), runtime.GOOS, runtime.GOARCH)
	if v.Commit != "" {
		log.Infof("  Git commit:", v.Commit)
	}

	if r, err := readOsRelease(); err == nil {
		log.Infof("")
		log.Infof("System information:")
		log.Infof("  Platform:", r.Name)
		log.Infof("  Platform version:", r.DisplayVersion())
	}
}
